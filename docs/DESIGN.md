# Design Document: Which Tokens Drive Emergent Misalignment

## 1. Purpose

This document is the method contract for **Which Tokens Drive Emergent Misalignment**. It specifies what is
...
One-liner: Attribute emergent misalignment to token-level patterns in fine-tuning documents on small models.

Hypothesis: Attribute emergent misalignment to token-level patterns in fine-tuning documents on small models.
...
|---|---|---|
| Config schema | `emtoken.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `emtoken.configs.loader` | Hydra compose API |
| Device | `emtoken.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `emtoken.models.registry` | Pinned revisions + hardware fit |
| Hooks | `emtoken.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `emtoken.models.generation` | Chat template path recording |
| Data | `emtoken.data` | Synthetic items, splits, manifests |
| Evaluation | `emtoken.evaluation` | Metrics, failure harness, protocols |
| Ablation | `emtoken.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `emtoken.reporting` | Aggregate → MD/TeX/figures |
| Cache | `emtoken.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/em-token-attribution
...
`src/emtoken/emtoken/` are preserved across regeneration.
